...
def compare_images(im1, im2, change_threshold=0.30):
    ...
    _, diff_thresh = cv2.threshold(diff, 28, 255, cv2.THRESH_BINARY)
    
    num_changed_pixels = np.sum(diff_thresh > 0)
    total_pixels = diff_thresh.size
    change_percentage = num_changed_pixels / total_pixels
...
while True:
    ...
    if frame_number%step_size==0 and prev_frame is not None:
        for index,space in enumerate(spaces):
            ...
            diffs[index]=compare_images(space_cropped,prev_frame[y1:y1+h,x1:x1+w,:])
    ...
    if frame_number%step_size==0:
        if prev_frame is None:
            ...
            fr=[j for j in np.argsort(diffs) if diffs[j]==True]
